...
void setup() {
  size(1200, 1000);
  Trash crumpledpaper = new Trash(width/2, 70, loadImage("1.png"), false, "Paper");
...
void draw() {
  background(255);
  for (int i = 0; i < listofTrash.size (); i++) {
    Trash trash = listofTrash.get(i);
    trash.display();
...
class Trash {
  ...
  void display() { 
    if (inabin()) {
      image (this.image, width+width, height+height, width/10, height/10);
    } else {
  ...
  Boolean inabin() {
    if ((this.x >= 0) && (this.x <= width/4) && (this.y >= 0) && (this.y <= height/4)) { 
      return true;
    } else if ((this.x >= 0) && (this.x <= width/4) && (this.y >= height/2) && (this.y <= (height/2)+height/4)) {
      return true;
    } else if ((this.x >= 0) && (this.x <= width/4) && (this.y >= height/5) && (this.y <= (height/5)+height/4)) {
      return true;
    } else if ((this.x >= width-width/4) && (this.x <= width) && (this.y >= 0) && (this.y <= height/4)) {
      return true;
    } else if ((this.x >= width-width/4) && (this.x <= width) && (this.y >= height/2) && (this.y <= (height/2)+height/4)) {
      return true;
    } else if ((this.x >= width-width/4) && (this.x <= width) && (this.y >= height/5) && (this.y <= (height/5)+height/4)) {
      return true;
    } else {
      return false;
    }
